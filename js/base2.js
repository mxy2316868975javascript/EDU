 ...
 */
myReady(function () {
    var ranking = getByClassName(document, "ranks")[0];
    ...
    // console.log(_data);
                ...
                for (var i = 0; i < _data.length; i++) 
                {
                    ...
                    hotListImg.src = _data[i].smallPhotoUrl;
                    hotListImg.alt = _data[i].name;
                    //名字
                    var hotListTitle = document.createElement("a");
                    hotListTitle.href = _data[i].providerLink;
                    var titleContent = document.createTextNode(_data[i].name);
                    hotListTitle.appendChild(titleContent);
                    //学习人数
                    var hotPrice = document.createElement("div");
                    var hotPriceContent = document.createTextNode(_data[i].price);
                    //图标..
                ...
                function move() {
                    ...
                    var hot_list = getByClassName(document, "ranks")[0];
                    ...
                    console.log(asideLi[0].offsetHeight);
                }
...
myReady(function () {
    ...
    var removeDefine = getByClassName(follow, "remove-fork")[0];
    var loginWrap = $("mask");
    var loginClose = $("mask-close");
    var user = $("form").getElementsByTagName("input")[0];
    var pass = $("form").getElementsByTagName("input")[1];
    var obtn = getByClassName($("form"), "login")[0];
    var loginError = getByClassName($("form"), "login-error")[0];
    ...
    //关注层显示
    function followShow() {
        var followRemove = getByClassName(follow, "remove-fork")[0];
        showHide(followAdd);
...
myReady(function () {
    ...
    var typeNumber = 10;
    var tabDesign = getByClassName(document, "tab")[0].getElementsByTagName("a")[0];
    var tabLanguage = getByClassName(document, "tab")[0].getElementsByTagName("a")[1];
    //
    ...
    function tabChange() {
        ...
        var child = getByClassName(parent, "lists");
        var i = 0;
     ...
     * 页码切换部分.
     */
    var pagination = getByClassName(document, "pages")[0];
    var paginationUP = getByClassName(pagination, "pageprev")[0];
    var paginationDOWN = getByClassName(pagination, "pagenext")[0];

    //数字切换页码
    // var pageUl = getByClassName(document, "page-ul")[0];
    var pageLi = getByClassName(pagination, "pagei");
    for (var i = 0; i < pageLi.length; i++) {
        pageLi[i].onclick = function () {
            for (var j = 0; j < pageLi.length; j++) {
                pageLi[j].className = "pagei f-fl";
            }
            //当前高亮
            this.className = "page-active pagei f-fl";
            //从服务器获取的页码
    ...
    function upDown() {
        for (var i = 0; i < pageLi.length; i++) {
            ...
            pageLi[i].className = "pagei f-fl";
        }
        if (parseInt(pageNoNumber % 8) == 0) {
            pageLi[pageLi.length - 1].className = "page-active pagei f-fl";

        } else {
            pageLi[parseInt(pageNoNumber % 8) - 1].className = "page-active pagei f-fl";
        }
        tabChange();
    }
    //向上翻页点击事件
    ...
    //Ajax函数
    function course() {
        var senddata = {pageNo: pageNoNumber,psize: psizeNumber,type: typeNumber};
        Ajax("get", "http://study.163.com/webDev/couresByCategory.htm", senddata,
            function (str) 
            {
                ...
                for (var i in list) {
                    ...
                    //课程发布者
                    var classProvider = document.createElement("span");
                    ...
                    if (list[i].price == "0") {
                        ...
                        // classPriceIcon.className = "iconfonts";
                    ...
                    hoverDiv.className = 'listhover';
                    
                        //课程图片
                    var hoverDivImg = document.createElement("img");
                    hoverDivImg.src = list[i].middlePhotoUrl;
                    hoverDivImg.alt = list[i].name;
                ...
                //     //hover时课程标题
                    ...
                    //分类
                    var hoverCategory = document.createElement("strong");
                ...
                //     //课程描述......
                    var hoverDescription = document.createElement("span");
                    ...
                    hoverDiv.appendChild(hoverDescription);
                //     hoverDescription.className = "description";
                ...
                for (var i = 0; i < classlisthover.length; i++) 
                {
                    ...
                    myAddEvent(classlisthover[i], "mouseout", function () {
                        ...
                        getByClassName(this, "listhover")[0].style.display = "none";
 ...
 */
myReady(function () {
    var moivePlayer = getByClassName(document, "source")[0].getElementsByTagName("img")[0];
    var playerWrap = getByClassName(document, "movie-wrap")[0];
    var closeMoive = $("movie-close");
    var moive = $("movie");
